...
- **Q-1.7-WATCHDOG.** An in-process reconnect watchdog on `Supervisor` conflicts with `mcp.client.stdio`'s anyio-taskgroup cancel scopes (cross-task exit of stdio_client context raises "Attempted to exit cancel scope in a different task"). M0 1.7 ships OS-level unit templates (`deploy/units/`) instead. In-process reconnect needs a per-upstream owning task pattern; deferred — file as post-M0 follow-up if `engram.health` degraded events become common in practice.
